...
async def publish_event(event_type: str, data: dict):
    ...
    Log locally instead of crashing when AWS credentials are invalid.
    """
    try:
        if USE_AWS:
            ...
            async with session.client("sqs", region_name=AWS_REGION) as sqs:
                async with session.client("events", region_name=AWS_REGION) as eventbridge:
                    ...
                    logger.info(f"[DRIVER] Sent to SQS: {event_type}")
                    ...
                    logger.info(f"[DRIVER] Sent to EventBridge: {event_type}")
        else:
